...
for idim = 1:n_dims
	...
	for ii = 1:numel(to_plt)
		...
		case 'sep'
			if size(data,2) > 1
				h(idim).h_sep = plot(ax,x,data','-','Marker','none','Color',[Color(idim,:) 0.1],'LineWidth',1);
			else
				h(idim).h_sep = my_scatter(x,data,ax,'.','MarkerEdgeColor',Color(idim,:),'MarkerEdgeAlpha',0.1);
			end
		case 'shade'
			h(idim).shade = fill(ax,[x flip(x)],[M+V, flip(M-V)],Color(idim,:),'FaceAlpha',0.1,'EdgeColor','none');
		case 'err'
			h(idim).err = errorbar(ax,x,M,V,'.-','Color',Color(idim,:),'LineWidth',2);
		case 'mean'
			h(idim).h_m   = plot(ax,x,M,'Color',Color(idim,:),'LineWidth',2);
		end
	end
end
