function [wU,wVt,wwfilts,to_save] = bilinearMultifiltRRR_coordAscent(Xin,Yout,rnks,lambda,opts)
...
% if rcond(XX)<1e-10
    % wridge = pinv(XX)*XY; % ridge regression solution
% else
...
% save test_rcond_nonsingular
% load test_rcond_nonsingular
% load test_rcond
% examine singular XX
% ax = np(2,2);arrayfun(@(ii) imagesc(ax(ii),XXc{ii}), 1:4); arrayfun(@(h) colorbar(h),ax); ef;
